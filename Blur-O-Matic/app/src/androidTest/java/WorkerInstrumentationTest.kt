...
class WorkerInstrumentationTest {
    ...
    @Test
    fun saveImageToFileWorker_doWork_resultSuccessReturnUrl() {
        ...
        runBlocking {
            val result = worker.doWork()
            val resultUrl = result.outputData.getString(KEY_IMAGE_URI)

            assertTrue(result is ListenableWorker.Result.Success)
            assertTrue(result.outputData.keyValueMap.containsKey(KEY_IMAGE_URI))
            assertTrue(
                resultUrl?.startsWith("content://media/external/images/media/")
                    ?: false
            )
        }
    }
